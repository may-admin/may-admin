...
K.options = {
	...
	htmlTags : {
		...
		pre : ['id', 'class'],
		code: ['id', 'class', '.background-color', '.color', '.font-family', '.font-size'],
				...
				yesBtn : {
					...
					click : function(e) {
						var type = K('.ke-code-type', dialog.div).val(),
							code = textarea.val(),
							cls = type === '' ? '' :  ' lang-' + type,
							html = '<pre class="prettyprint' + cls + '"><code>\n' + K.escape(code) + '</code></pre> ';
						if (K.trim(code) === '') {
		...
		self.plugin.multiImageDialog({
			clickFn : function (urlList) {
				...
				K.each(urlList, function(i, data) {
					if (self.afterUpload) {
						self.afterUpload.call(self, data.link, data, 'multiimage');
					}
					self.exec('insertimage', data.link, data.title, data.width, data.height, data.border, data.align);
				});
				setTimeout(function() {
					self.hideDialog().focus();
				}, 0);

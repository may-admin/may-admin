...
class Select
{
    ...
    public function index($data, $wconfig)
    {
        ...
        if ( !isset($wconfig['from']) || !in_array($wconfig['from'], ['selectlist', 'function', 'options']) ){
            return '<div class="form-group"><label class="col-sm-'.$wconfig['title_col'].' control-label">'.$wconfig['title'].'</label><div class="col-sm-'.$wconfig['content_col'].'"><span class="help-block">select来源 from 必须是 selectlist 、 function 或 option</span></div></div>';
        }

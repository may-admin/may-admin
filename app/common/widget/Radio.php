...
class Radio
{
    ...
    public function index($data, $wconfig)
    {
        ...
        if ( !isset($wconfig['from']) || !in_array($wconfig['from'], ['selectlist', 'function', 'options']) ){
            return '<div class="form-group"><label class="col-sm-'.$wconfig['title_col'].' control-label">'.$wconfig['title'].'</label><div class="col-sm-'.$wconfig['content_col'].'"><span class="help-block">radio来源 from 必须是 selectlist 、 function 或 options</span></div></div>';
        }
        if ( !isset($wconfig['fromcfg']) || empty($wconfig['fromcfg'])){
            return '<div class="form-group"><label class="col-sm-'.$wconfig['title_col'].' control-label">'.$wconfig['title'].'</label><div class="col-sm-'.$wconfig['content_col'].'"><span class="help-block">radio数据来源 fromcfg 不能为空</span></div></div>';
        }
        ...
        return View::fetch('common@widget/radio');
    }
}